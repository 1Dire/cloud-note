        ...
        <div className="prose prose-lg mx-auto px-4 dark:prose-invert leading-relaxed tracking-wide w-full max-w-4xl lg:w-4xl">
            ...
            <div>
                ...
                <ul className="list-disc list-inside mt-3 space-y-1">
                    <li><strong>테인트</strong>: <em>노드에 붙는</em> “거부 규칙”. 일치하는 톨러레이션이 없으면 파드를 <strong>받지 않음</strong>.</li>
                    <li><strong>톨러레이션</strong>: <em>파드에 붙는</em> “허용 증명서”. 특정 테인트를 <strong>허용</strong>해 그 노드에 스케줄링/유지 가능.</li>
                    <li><strong>어피니티와의 관계</strong>: 반대가 아니라 <strong>다른 축</strong>. 어피니티는 “가고 싶은 곳/피하고 싶은 곳”, 테인트는 “여긴 아무나 못 와”. 함께 쓰면 가장 깔끔함.</li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul className="list-disc list-inside mt-3 space-y-1">
                    <li><code>NoSchedule</code> : 일치하는 톨러레이션 없으면 <strong>새 파드 배치 금지</strong>.</li>
                    <li><code>PreferNoSchedule</code> : 가능하면 피하지만 <strong>강제 금지는 아님</strong>.</li>
                    <li><code>NoExecute</code> : 일치하는 톨러레이션 없으면 <strong>기존 파드도 축출</strong>. <code>tolerationSeconds</code>로 유예 가능.</li>
                </ul>
            </div>
            ...
            <div>
                ...
                <BlockCode language="bash">{`# 노드에 테인트 추가 (key=value:Effect)
...
kubectl taint nodes <NODE_NAME> node-role.kubernetes.io/master:NoSchedule-`}</BlockCode>
            ...
            <div>
                ...
                <BlockCode language="yaml">{`apiVersion: v1
kind: Pod
metadata:
  ...
  containers:
    - name: app
      image: nginx`}</BlockCode>
                ...
                <BlockCode language="yaml">{`tolerations:
    ...
    tolerationSeconds: 600`}</BlockCode>
            ...
            <div>
                ...
                <ol className="list-decimal list-inside space-y-2 mt-2">
                    <li>
                        노드에 테인트 걸기
                        <BlockCode language="bash">{`kubectl get nodes
kubectl taint nodes <NODE> dedicated=team-a:NoSchedule`}</BlockCode>
                    </li>
                    <li>
                        톨러레이션 <strong>없는</strong> 파드 배포 → 해당 노드에 스케줄 안 됨
                        <BlockCode language="bash">{`kubectl run no-toleration --image=nginx --restart=Never`}</BlockCode>
                    </li>
                    <li>
    ...
    - name: app
      image: nginx`}</BlockCode>
                    ...
                    <li>
                        ...
                        <BlockCode language="bash">{`# 예) not-ready 상태와 유사한 NoExecute 테인트 임의 부여
kubectl taint nodes <NODE> node.kubernetes.io/not-ready:NoExecute`}</BlockCode>
                    ...
                    <li>
                        정리
                        <BlockCode language="bash">{`kubectl taint nodes <NODE> dedicated=team-a:NoSchedule-
kubectl taint nodes <NODE> node.kubernetes.io/not-ready:NoExecute-`}</BlockCode>
                    </li>
                </ol>
            </div>
