...
data:
  db-password: cGFzc3dvcmQ=  # base64 인코딩된 "password"`}
        ...
        <BlockCode
          language="bash"
          code={`kubectl create -f wordpress/wordpress-secret.yml`}
        />
      </div>
      ...
      <div>
        ...
        <BlockCode
          language="yaml"
          code={`# single-deployment-no-volumes.yml
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  template:
    ...
    spec:
      ...
      - name: mysql
        ...
        <BlockCode
          language="bash"
          code={`kubectl create -f wordpress/single-deployment-no-volumes.yml`}
        />
      </div>
...
spec:
  ...
  - protocol: TCP
    ...
    nodePort: 30080`}
        ...
        <BlockCode
          language="bash"
          code={`kubectl create -f wordpress/wordpress-service.yml
minikube service wordpress-service --url`}
      ...
      <div>
        ...
        <p>
          현재는 <strong>볼륨 미사용 상태</strong>이기 때문에, 워드프레스에
          저장된 데이터는 <strong>파드 재시작 시 모두 사라집니다.</strong>
        </p>
        <p>재시작 예시:</p>
        <BlockCode
          language="bash"
          code={`kubectl delete pod [wordpress-pod-name]`}
        />
      ...
      <div>
        ...
        <p className="mt-2">
          실습 경로: <code>/kubernetes-course/wordpress/</code>
        </p>
      </div>
    </div>
  );
